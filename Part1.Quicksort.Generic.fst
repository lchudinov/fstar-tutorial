...
let rec partition (#a:Type) (f:total_order_t a) (l: list a) (pivot: a)
  : x:(list a & list a) {length (fst x) + length (snd x) = length l}
  = match l with
  | [] -> [], []
  | hd::tl ->
    let l1, l2 = partition f tl pivot in
    if f pivot hd 
    then hd::l1, l2
    else l1, hd::l2
...
let rec sort #a (f:total_order_t a) (l:list a)
  ...
  = match l with
    | [] -> []
    | pivot :: tl ->
      let hi, lo = partition f tl pivot in
      append (sort f lo) (pivot :: sort f hi) 

let rec partition_mem (#a:eqtype) (f:total_order_t a) (l:list a) (pivot: a)
  : Lemma (let l1, l2 = partition f l pivot in
           (forall x. mem x l1 ==> f pivot x) /\
           (forall x. mem x l2 ==> not (f pivot x)) /\
           (forall x. mem x l = (mem x l1 || mem x l2)))
  = match l with
  | [] -> ()
  | hd :: tl -> partition_mem f tl pivot
